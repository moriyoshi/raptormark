...
// which is complete and self-describing (see
// .agents/docs/LTM/runtime-metadata-producers.md). Keep
...
type Options struct {
	...
	ExecMap []byte
	// DlMap, if non-empty, is placed at DlPath. It is what lets a guest dlopen
	// of a `.so` path reach the UNIT that serves it; without it the runtime has
	// no unit to resolve to and the dlopen fails, which is what a real loader
	// does for an absent plugin. Encode it with internal/link.DlMap.
	//
	// Independent of ExecMap on purpose: an image may have units and no
	// alternate entry points, or several programs and no plugins.
	DlMap []byte
...
func Build(root string, opts Options) ([]byte, Stats, error) {
	...
	if len(opts.DlMap) > 0 {
		if err := CheckDlMap(root, opts.DlMap); err != nil {
			return nil, st, err
		}
		addFile(tree, DlPath, opts.DlMap, &st)
	}
