...
func TestClosureFollowsScriptsAndBareWords(t *testing.T) {
	inv, _ := inventory(t)
	got, err := Closure(inv, ClosureOptions{
...
func TestClosureExcludeAndMax(t *testing.T) {
	...
	capped, err := Closure(inv, ClosureOptions{
		Seeds:    []string{"/entry.sh"},
		PathDirs: []string{"/usr/bin"},
		Max:      1,
	})
...
func TestClosureRejectsUnresolvableSeeds(t *testing.T) {
	...
	if _, err := Closure(inv, ClosureOptions{Seeds: []string{"/nope"}}); err == nil {
...
func TestSymlinkedEntrypointResolves(t *testing.T) {
	...
	closure, err := Closure(inv, ClosureOptions{Seeds: got, PathDirs: []string{"/usr/local/bin", "/usr/bin"}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPathLookupDoesNotStopAtAWrapper(t *testing.T) {
	...
	closure, err := Closure(inv, ClosureOptions{Seeds: EntrypointSeeds(cfg, inv), PathDirs: dirs})
	if err != nil {
		t.Fatal(err)
	}
