...
// promoted -- and `RAPTORMARK_STABLE_SPLIT` was the one README item 1 reduced to
// promoting. The same comment names the precondition: "Anything here that
// becomes a shipping default has to move onto Request and into ObjectKey first,
// or the cache will serve the wrong object."
//
// ✅ THAT PROMOTION HAPPENED on 2026-08-23. The switch in the list below is now
// its inverse, `RAPTORMARK_NO_STABLE_SPLIT`, which is the shape a promoted
// switch leaves behind: the default contributes NOTHING to the key (see
// `ExperimentalSettings`, and the pinned literal in the tests, both of which are
// unchanged by the flip), and turning the default OFF is what has to move it.
//
...
var experimentalVars = []string{
	// Restores llvm-split. The content-stable partitioner is the default since
	// 2026-08-23 -- the promotion this file was written to unblock -- and the two
	// assign differently, so flipping back means different partitions and a
	// different object.
