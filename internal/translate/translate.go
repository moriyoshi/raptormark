...
type Options struct {
	...
	// SuspendViaCall has elflift emit the suspend check as a CALL to
	// `_ecv_suspended` instead of a read of the `__ecv_unwinding` wasm global
	// (elfconv patch 0067, which gates patch 0059's optimization).
	//
	// ❗ REQUIRED BY THE `wasix` PROFILE and pointless elsewhere. WASIX's
	// dynamic linker refuses a module that imports a GLOBAL from `env`, and
	// `__ecv_unwinding` is one -- measured: "Expected import to be a function:
	// 'env'.__ecv_unwinding". The call form costs a call at every suspend-check
	// site (33,540 on a linked bash module), which is exactly what 0059 removed,
...
func (b Builder) TranslateID(elfSHA256 string, opts Options) string {
	...
	// Appended only when set, for the same reason: a default translation must
	// key exactly as it always has. It changes the emitted bytes at every
	// suspend-check site, so it MUST be in the key -- an object lifted with the
	// call form is not interchangeable with one lifted with the global read.
	if opts.SuspendViaCall {
		parts = append(parts, "suspend_via_call=true")
	}
...
// already absorbed: under the stable partitioner (the default since 2026-08-23,
// ECV_STABLE_SPLIT when this was written) every partition but the one holding
...
func (r Request) args() []string {
	...
	if o.SuspendViaCall {
		args = append(args, "--suspend-via-call")
	}
	if r.Jobs > 0 {
...
func (b Builder) Run(ctx context.Context, r Request) error {
	...
	// THE SAME SHAPE, for the same reason. Content-stable partitioning
	// (ecv-split) is the DEFAULT since 2026-08-23 -- the promotion
	// `experimental.go` was written to unblock -- so what remains is the switch
	// that turns it back off. It changes the emitted object, so unlike the
	// merged-prepare inverse it is ALSO in `experimentalVars`: absent, the key
	// describes the default, and a build that flips it must not collide with one
	// that did not.
	//
	// RAPTORMARK_STABLE_SPLIT is gone rather than kept as a no-op alias. A stale
	// shell still exporting it now asks for exactly what it gets, and leaving it
	// in the identity list would have given two cache keys for one object.
	if os.Getenv("RAPTORMARK_NO_STABLE_SPLIT") != "" {
		docker = append(docker, "-e", "ECV_NO_STABLE_SPLIT=1")
	}
...
type LinkRequest struct {
	...
	SideOut string
	// Profile selects which ecvisor archive link-all links, and therefore the
	// module's network backend, its re-entrant surface and its loader backend.
	// Empty means link-all's default (`wasmedge`), which is what every shipping
	// artifact uses.
	//
	// ⚠️ It applies to the SUPERVISOR as well as the flat module. That was not
	// true until 2026-08-24: `supervisorLinkArgs` hardcoded the default archive,
	// so `--side-out --profile hosted` produced a supervisor with no loader
	// backend and none of the exports a host drives. See
	// TestSupervisorHonoursTheProfile.
	Profile string
}
