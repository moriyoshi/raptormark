...
func (c *TranslateOne) lift(bc string) error {
	return run(elfconvRoot+"/build/lifter/elflift", c.liftArgs(bc)...)
}
...
// TestPICObjectsStillLinkFlat guards it.
//
...
func (c *TranslateOne) codegenEcvisor(sdk, bc, obj string, libBCs []string) error {
	...
	if mergedPrepareEnabled() {
		...
		// split. Only available with ECV_STABLE_SPLIT, because llvm-split is an
