...
//     this list exists to prevent. It also holds the partitioner default
//     (ECV_NO_STABLE_SPLIT since 2026-08-23, ECV_STABLE_SPLIT before it), so it
//     belonged here already — and that membership is what makes flipping that
//     default safe: the flip changes this hash, so objects cached under the old
//     partitioner miss instead of being served.
//
...
//   - ecv-split.cpp     — the partitioner (default)
...
// affordable.
//
// ⚠️ THE BAZEL FILES JOINED THE LIST ON 2026-08-23, and the reason is the exact
// caveat this comment used to carry about the Dockerfile.
//
// It used to read: "builder/Dockerfile is excluded ... with one caveat worth
// knowing: it carries the compile flags for the tools above, so rebuilding them
// at a different optimisation level would not move the key. It is not in the
// list because it also carries the ecvisor runtime build, which changes often
// and reaches only the final link."
//
// Both halves of that stopped being true at once. The Dockerfile no longer
// builds anything -- it is `COPY . /` over //builder:stage -- so it no longer
// carries the runtime build that kept it out. And the compile flags it did
// carry are now in Bazel, where the caveat would apply unchanged:
//
//   - bazel/llvm_tool.bzl   — `-std=c++17 -fno-rtti -O2` and the llvm-config
//     invocation. Change `-O2` here and every tool
//     emits different bytes.
//   - builder/BUILD.bazel   — the llvm-config COMPONENT list per tool, which is
//     the link line.
//   - bazel/sdk.bzl         — WHICH LLVM they link against.
//
// builder/BUILD.bazel also holds the staging genrule and the test definitions,
// so editing those invalidates every cached object too. That is over-
// invalidation and it is accepted deliberately, on this file's own rule below:
// over-invalidating costs CPU, under-invalidating serves a stale object for a
// pipeline that no longer produces it.
//
// STILL EXCLUDED, and for reasons that did not change: bazel/wasi_object.bzl,
// bazel/ecvisor.bzl and bazel/crates.bzl build the C shims and the ecvisor
// archives, which reach only the FINAL LINK and never a translated object --
// the same reason `runtime/` has never been in this list, and the reason
// CLAUDE.md can say "if only runtime/ changed, relink, do not re-lift".
//
...
// changed, and neither can change a cached object.
//
...
var translateSources = []string{
	...
	// How the four tools above are COMPILED. These carried the same weight as
	// the sources from the moment the recipes left builder/Dockerfile.
	"bazel/llvm_tool.bzl",
	"builder/BUILD.bazel",
	"bazel/sdk.bzl",
}
