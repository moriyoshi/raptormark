...
pub unsafe extern "C" fn __remill_jump(
    ...
    // ⚠️ DIAGNOSTIC, gated. `__remill_jump` is where a `br` to a target elflift
    // did not discover for the branching function lands -- which is every
    // `longjmp`, because `__longjmp`'s `br x30` target is a return address in
    // some other function. The indirect branch is implemented as a nested CALL
    // into the containing function, so the target runs INSIDE the jumping
    // function's frame, and callee-saved state has to travel through `*state`.
    // Whether it does is the open question in the setjmp/longjmp investigation
    // (`.agents-workspace/longjmp/`), and nothing could see it: the existing
    // report prints registers only on a MISS, and this case is a HIT.
    {
        let g = &(*state).gpr;
        // ⚠️ `ecv_debug!`, NOT `eprintln!`. A raw print here is reserved for the
        // fatal path, and `diag`'s `only_the_fatal_path_prints_without_going_
        // through_the_sink` counts those sites and fails when one appears --
        // which it did, on this line, the first time it was written.
        ecv_debug!(
            ecv,
            "JMP t_vma={t_vma:#x} lr={:#x} x0={:#x} x19={:#x} x20={:#x} sp={:#x}",
            g.x[30].val,
            g.x[0].val,
            g.x[19].val,
            g.x[20].val,
    ...
    // A `br` into a function that is ALREADY on the guest stack is a non-local
    // jump -- a `longjmp` -- and the nested-call dispatch below cannot express
    // it. Calling `f` here runs the target INSIDE the jumping frame, so when it
    // returns, every frame the `longjmp` abandoned resumes: measured on
    // `__libc_siglongjmp`, whose instruction after `bl __longjmp` is the
    // mask-restore block, which branches back to the call. See
    // `nonlocal_jump_depth` and JOURNAL.md 2026-09-01.
    //
    // The inline call-history fast path bumps the length without Rust seeing
    // it, so the depth has to be adopted before it is searched -- the same
    // reason the syscall trampoline adopts. Guarded by the entry check so an
    // ordinary tail call pays only a binary search.
    if (*ctx).func_at(t_vma).is_none() {
        (*ctx).adopt_call_history_depth();
        if let Some(depth) = (*ctx).nonlocal_jump_depth(t_vma) {
            ecv_debug!(
                ecv,
                "JMP non-local to {t_vma:#x}, unwinding to call_history depth {depth} (of {})",
                (*ctx).call_history.len()
            );
            (*state).gpr.pc.val = t_vma;
            (*ctx).begin_nonlocal_jump(t_vma, depth);
...
pub unsafe extern "C" fn _ecv_save_call_history(
    ...
    // ❗ NO adopt/publish HERE, AND THAT IS CORRECT -- but read on, because this
    // comment used to claim the opposite.
    //
    // It said: "The inline fast path may have pushed frames Rust has not seen;
    // adopt them before adding ours, and republish afterwards in case the push
    // moved the buffer." That describes `_ecv_save_call_history_ic`, which
    // brackets a call to THIS function. The bracketing moved there when the two
    // entry points were split; the comment did not.
    //
    // Why the plain entry point needs no brackets: lifted code only calls it
    // when the module was built WITHOUT the inline fast path, and such a module
    // leaves `ecv_ch_built` at 0, so `ch_inline` is false and both bracket calls
    // are no-ops anyway. A module built WITH it emits `_ecv_save_call_history_ic`
    // instead.
    //
    // ⚠️ Corrected 2026-08-25 while auditing the adopt/publish invariant, which
    // `.agents/docs/TODO.md` records as having no enforcement and three holes
    // found in three places. A comment asserting the invariant is honoured where
    // it is not would end that audit early at exactly this function -- which is
    // how that entry says the second and third holes survived the first.
