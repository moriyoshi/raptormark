...
//! [pieExeBase 0x100000, BRK_START 0x0C00_0000) = 191 MiB (must hold the whole
//! prelinked closure — large glibc apps like postgres pull ~150 MiB of objects),
//! then brk [192 MiB, 200 MiB), mmap [200 MiB, 352 MiB), guest stack top at
//! 384 MiB. Grown from 256 MiB (low region was only 63 MiB) to fit big closures.
//!
//! ❗ **GROWN AGAIN 2026-08-27, ON THE OPERATOR'S DECISION: BRK_START
//! 0x0A00_0000 -> 0x0C00_0000, so the fused region goes 156 -> 188 MiB** (the
//! fuser measures it from `defaultExeBase` 0x400000, not from `pieExeBase`).
//! `raptormark build postgres:17` and `:18` BOTH overflowed it -- measured
//! needing `0xb020010` and `0xb690010` against a `0xa000000` ceiling, 16.1 and
//! 22.6 MiB over -- and every program then packs independently, losing all
//! cross-program library sharing.
//!
//! ⚠️ **THE ARENA DID NOT GROW, AND MUST NOT HAVE.** See the CEILING note below:
//! `arena_size × (live + suspended) < 4 GiB`, 384 MiB allows 10 process buffers
//! and postgres needs 7 concurrently for a single psql. Buying the fused region
//! out of the arena would have spent exactly the headroom this change exists to
//! serve.
//!
//! Where the 32 MiB came from instead, and why each was affordable:
//!   * **brk 32 -> 8 MiB.** The 2026-08-25 measurement below stands: max brk
//!     high-water across 104 guest runs was 1,164 KiB, and 103 of 104 were at
//!     136 KiB. 8 MiB is still **7x the largest ever observed**, and overflow is
//!     graceful -- `NR_BRK` returns the break unchanged, glibc yields ENOMEM,
//!     malloc switches to mmap, which is the path initdb already takes.
//!   * **mmap 160 -> 152 MiB.** Peak private mmap measured 88 MiB, so this is
//!     still 1.7x it, and it remains far above the 96 MiB that prompted the
//!     rebalance.
//!
//! ✅ **RAISING this ceiling cannot invalidate a cached object.** `layout.go`
//! uses it only as a refusal test (`if next > brkStartVMA`), so every closure
//! that already fit places every library at the same base and fuses to the same
//! bytes. Only closures that previously OVERFLOWED change -- they stop falling
//! back to per-image packing, which is the point. `BRK_END_VMA` and
//! `MMAP_START_VMA` are pure ecvisor policy and appear nowhere outside
//! `runtime/`, so they cost nothing at all.
//!
//! ⚠️ The line below saying the prelinker's `arenaBrkStart` must match is STALE:
//! `internal/prelink` does not exist in this tree. The live cross-language guard
//! is `TestBrkStartMatchesTheRuntime` (`internal/fuse/layout_test.go`), which
//! scans this file.
//!
//! ❗ **REBALANCED 2026-08-25: brk 96 -> 32 MiB, mmap window 96 -> 160 MiB.**
//! The arena did not grow; the boundary between two of its regions moved.
//! `.agents/docs/TODO.md` carried a ⭐ decision about the mmap window being
//! starved -- private `mmap` and `MAP_SHARED` compete for one 96 MiB span, so a
//! large `shared_buffers` leaves malloc's fallback ~20 MiB. The four options
//! considered for three sessions were grow the arena, split the window, make
//! reservations lazy, or accept the ceiling. None noticed the window's
//! NEIGHBOUR was nearly dead.
//!
//! Measured across **104 guest runs** of the fast e2e suite: max brk high-water
//! **1,164 KiB of 96 MiB (1.2%)**, with 103 of 104 runs at 136 KiB. Peak private
//! mmap was 88 MiB of 96 -- 91.7% of the region next door.
//!
//! ⚠️ **Shrinking brk is safe BECAUSE OVERFLOW IS GRACEFUL, and postgres already
//! relies on it.** `sys.rs`'s `NR_BRK` returns the break unchanged when a
//! request leaves the region, which is how Linux reports failure; glibc turns
//! that into ENOMEM and malloc switches to mmap. initdb's post-bootstrap backend
//! already asks for ~190 MiB of brk and already takes that path. Its demand
//! could never be met by ANY brk size, so brk size is close to irrelevant to it
//! while mmap size is exactly what it needs.
//! ❌ Do not read the 32 MiB as tight: it is 27x the largest brk high-water ever
//! measured here, and a guest that exceeds it lands in the region this change
//! just grew by 64 MiB.
//! (2026-08-27: brk is now 8 MiB and mmap 152 MiB -- see the note at the top.
//! The reasoning is unchanged and the margin is 7x rather than 27x.)
...
pub const BRK_START_VMA: u64 = 0x0C00_0000;
pub const BRK_END_VMA: u64 = 0x0C80_0000;
pub const MMAP_START_VMA: u64 = 0x0C80_0000;
...
/// Where the intercepted `dlerror` keeps its message.
///
/// `dlerror` must hand the guest a POINTER to a NUL-terminated string, so the
/// text has to live in the arena; a Rust-side `String` cannot be returned. It
/// goes in the low per-process sliver, beside the pthread and musl bring-up
/// structures, for the reason given at `MUSL_TLS_MODULES_VMA`: this is
/// per-process state (two processes can have different last-errors), and a
/// bounded snapshot already carries this range, so it needs no new plumbing.
///
/// Placed in the measured GAP between the musl module list, which ends at
/// `THREAD_PTR - 0x900`, and glibc's `_dl_stack_*` bring-up at
/// `0xff9a0..0xff9e0` (`THREAD_PTR - 0x660`). The const assertions below fail
/// the build if either neighbour grows into it -- silence there would be a
/// message overwriting libc's thread list.
pub const DLERROR_VMA: u64 = THREAD_PTR - 0x900;
pub const DLERROR_MAX: u64 = 0x100;

const _: () = assert!(
    MUSL_TLS_MODULES_VMA + MUSL_TLS_MODULES_MAX <= DLERROR_VMA,
    "the musl TLS module list would overlap the dlerror buffer"
);
const _: () = assert!(
    DLERROR_VMA + DLERROR_MAX <= THREAD_PTR - 0x660,
    "the dlerror buffer would overlap glibc's _dl_stack_* bring-up state"
);
const _: () = assert!(
    DLERROR_VMA >= LOW_PERPROCESS_FLOOR,
    "the dlerror buffer must sit inside the sliver a bounded snapshot carries"
);
...
impl Arena {
    /// Feeds the address-budget high-water marks.
    ///
    /// ⚠️ Reports BYTES USED per region, not raw VMAs, because the question it
    /// answers is "how much of each fixed region does a real guest need" -- and
    /// a VMA would have to be mentally rebased against a constant every time.
    /// The shared side is not visible from here (it lives in `ShmWindow`), so it
    /// is reported as 0 and filled in by `EcvContext::shm_reserve`.
    #[inline]
    fn note_budget(&self) {
        crate::diag::note_address_use(
            self.brk_cur.saturating_sub(BRK_START_VMA),
            self.mmap_cur.saturating_sub(MMAP_START_VMA),
            0,
        );
    }
}
...
pub struct Arena {
    ...
    /// however much arena it was given. Measured on initdb.
    ///
    /// ⚠️ This cited `.agents/docs/JOURNAL.md` for the full bump progression.
    /// That account is GONE from both JOURNAL.md and LTM/ as of 2026-08-25, so
    /// the ladder written out above is now the ONLY record of it. Do not trim
    /// it to a summary.
