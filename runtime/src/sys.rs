...
use crate::context::{
    ...
    Pending, Pipe, ShmFile, ShmSeg, SigAction, UnixListener,
};
...
fn svc_dispatch(state: &mut State, ctx: &mut EcvContext) {
    ...
    match nr {
        ...
        // the guest side (.agents/docs/JOURNAL.md).
        NR_SETUID => {
        ...
        // Intercepted dl* API (pseudo-syscalls; the prelinker stubbed the real
        // dlopen/dlsym/dlclose/dlerror). Everything is fused offline, so dlopen
        // returns a non-null sentinel handle (callers proceed), dlsym resolves
        // against the fused export table (.ecv.dlsyms), and dlclose/dlerror no-op.
        NR_ECV_DLOPEN => {
            // dlopen(path=x0, mode=x1): return a global sentinel handle so callers
            // proceed. A missing dlopen'd module still lets dlsym fail gracefully.
            if trace_log() {
                let p = state.arg(0);
                let name = if p == 0 {
                    b"NULL".to_vec()
                } else {
                    ctx.arena.read_cstr(p)
                };
                ecv_trace!(
                    ecvisor,
                    "dlopen({}) -> handle=0x1",
        ...
        NR_ECV_DLSYM => {
            // dlsym(handle=x0, name=x1): resolve against the fused export table.
            let name = ctx.arena.read_cstr(state.arg(1));
            let vma = ctx.dlsym_lookup(&name);
            ecv_trace!(
                ecvisor,
                "dlsym({}) -> 0x{vma:x}",
                String::from_utf8_lossy(&name)
        ...
        NR_ECV_DLCLOSE => state.set_ret(0),
        NR_ECV_DLERROR => state.set_ret(0),
        _ => {
...
/// Resolves a syscall path argument against a dirfd + the cwd. M1 supports
/// AT_FDCWD and absolute paths only.
fn resolve_arg(ctx: &EcvContext, dirfd: i64, path: &[u8], follow: bool) -> Option<Resolved> {
    if path.first() != Some(&b'/') && dirfd != AT_FDCWD {
        return None; // relative-to-arbitrary-dirfd unsupported yet
    }
    ctx.vfs.resolve(&ctx.cwd, path, follow)
}
    ...
    match resolved {
        Some(r) => match r.meta.kind {
            NodeKind::Dir => {
                ...
                let fd = ctx.alloc_fd(OpenFile::Dir { entries, pos: 0 });
                finish_open(state, ctx, fd, flags);
            }
            NodeKind::File => {
...
fn sys_unlinkat(state: &mut State, ctx: &mut EcvContext) {
    ...
    if ctx.vfs.resolve(&ctx.cwd, &path, false).is_none() {
        state.set_ret_err(ENOENT);
        return;
    }
    let abs = absolutize(&ctx.cwd, &path);
...
fn sys_getdents64(state: &mut State, ctx: &mut EcvContext) {
    ...
    let Some(OpenFile::Dir { entries, pos }) = ctx.fds.get_mut(fd).and_then(|s| s.as_mut()) else {
...
fn sys_readlinkat(state: &mut State, ctx: &mut EcvContext) {
    ...
    match ctx.vfs.readlink(&ctx.cwd, &path) {
        Some(target) => {
...
fn sys_mkdirat(state: &mut State, ctx: &mut EcvContext) {
    ...
    if ctx.vfs.resolve(&ctx.cwd, &path, false).is_some() {
        state.set_ret_err(EEXIST);
        return;
    }
    let abs = absolutize(&ctx.cwd, &path);
...
fn sys_execve(state: &mut State, ctx: &mut EcvContext) {
    let path = ctx.arena.read_cstr(state.arg(0));
    let argv = read_ptr_array(&ctx.arena, state.arg(1));
    let envp = read_ptr_array(&ctx.arena, state.arg(2));

    let idx = match ctx.programs.resolve(&ctx.vfs, &ctx.cwd, &path) {
        Some(i) => i,
        None => {
            // A path that exists but has no translated program is ENOEXEC
            // (e.g. a data file, a dynamic ELF, or a script — shebang support
            // is a later addition); a missing path is ENOENT.
