...
unsafe fn boot_world(argc: c_int, argv: *mut *mut c_char) -> i32 {
    ...
    // Select the entry program the boot argv names; single-program modules
    // (empty exec map) fall back to program 0.
    let entry_idx = argv_g
        .first()
        .and_then(|a0| programs.resolve(&vfs, &cwd, a0))
        .unwrap_or(0);
