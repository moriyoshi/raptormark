...
use super::{poll1, NetBackend, NetErr, NetHandle, Readiness, SockAddr, WaitOutcome};
...
impl NetBackend for HostWasiNet {
    ...
    fn ready(&mut self, h: NetHandle, want_write: bool) -> Readiness {
        ...
        //
        // ⚠️ THE ZERO IS PREVIEW1'S MEANING AND IS NOT PORTABLE. WASIX reads a
        // zero timeout as `Duration::MAX` and never returns; its immediate
        // probe is 1 ns. `net::wasix` therefore passes 1 here, and `poll1`
        // deliberately refuses to pick for either of them.
        let mut subs = [0u8; poll1::SUB_LEN * 2];
        poll1::fd_sub(&mut subs, 0, 0, h.0 as u32, want_write);
        poll1::clock_sub(&mut subs, 1, 1, 0);
    ...
    fn wait(&mut self, waiters: &[(NetHandle, bool)], timeout_nanos: Option<u128>) -> WaitOutcome {
        ...
        if let Some(t) = timeout_nanos {
            poll1::clock_sub(&mut subs, n, n as u64, t.min(u64::MAX as u128) as u64);
        }
