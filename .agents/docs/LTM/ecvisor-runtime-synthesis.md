...
differ and use `RAPTORMARK_ECV_SNAPCHECK=1` against the full-buffer oracle while
both paths exist.
...
The host Rust suite currently runs 47 tests. Also type-check the shipping target
and use env-gated E2E coverage for behavior. Timer, non-blocking, UDS, shared-file,
nginx, PostgreSQL, and released-shim tests observe different boundaries.
...
- Bounded snapshots remain opt-in; switch cost and nginx behavior remain open.
