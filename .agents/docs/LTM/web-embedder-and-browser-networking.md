...
`ecv_signal` queues host-requested signals between slices. Default terminating actions and unconditional SIGKILL are modelled; nginx tests still use its installed handlers for graceful control.
...
## Consolidated Update: Unit Coverage and Browser Matrix

Focused unit coverage now guards all four previously uncovered modules. `load.ts` observes Cache API traffic and graceful cache failure; `netv1.ts` guards synthetic-address reverse lookup; `files.ts` guards basename matching and independent descriptor offsets; and `sockets.ts` guards file-versus-socket routing and short reads. The web unit suite reached 104 tests after the host-stamp work.

Chromium runs all 14 browser specs when the generated bundle is current and mise's Node bin directory is on `PATH`. WebKit is downloaded but cannot launch on this host because its system libraries are absent; selecting it produces immediate host-dependency failures, not raptormark regressions. Firefox and WebKit remain open coverage in TODO. Rebuild `web/dist/raptormark.js` after any `web/src` change before interpreting browser results.
