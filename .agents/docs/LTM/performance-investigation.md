...
## Consolidated Update: Host Timing and Index-Shift Cost

`web/bin/run.ts --stamp PREFIX` timestamps completed guest marker lines with host `performance.now()` before echo or parsing work. This gives an external clock for benchmarks whose guest clock is the subject under test. A frozen-clock Wasm probe produced 0 ns/call inside the guest and 134 ns/call from the host, proving the two measurements are independent; the real lifted benchmark later reported agreeing values, as expected on a healthy clock.

Registry-index shifts are expensive at the whole-object layer but mostly absorbed by the partition cache. On busybox, index 0 cold took 19 s with 0/80 partition hits, while index 1 took 7 s with 76/80 hits and 0.2 s of codegen. The remaining cost is serial preparation; renaming the exported indexed symbol would invalidate all cached objects and was declined after measurement.

The undecoded census has no measurable cost when it records no sites. An apparent PostgreSQL slowdown was variance from two non-interleaved runs; interleaved Python and PostgreSQL arms showed identical output and no systematic penalty. Performance hypotheses must be removed or interleaved, not inferred from one before/after pair.
