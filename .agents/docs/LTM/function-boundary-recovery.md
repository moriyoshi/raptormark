...
- `.ecv.funcs` was an unused, incomplete inventory, not a safe restrictive keep-list. **REMOVED 2026-08-25** once measured to be redundant with the merged `.symtab`; see Details.
...
❗ **`.ecv.funcs` NO LONGER EXISTS. Removed 2026-08-25.** The fuser emitted it and
neither the lifter nor the patch series ever consumed it. On the fused PostgreSQL
image it was 942 KB of `SHF_ALLOC` payload and listed 6,151 functions where
elflift discovers 6,570; the missing 419 included PLT stubs and gap-filling rest
functions, so using it restrictively would have sent real entries to
`_ecv_unreached`.

**Why it was removed rather than repurposed**: measured across three fused
fixtures (two glibc, one musl), every address it emitted was present in the
merged `.symtab` with an IDENTICAL size, and it was never a superset. It was
redundant BY CONSTRUCTION -- `funcRangesOf` fed both tables, unified after they
had once disagreed by 2,531 functions, and that unification is what made the
second copy carry nothing new. Removing it reclaimed 332 KB on a real OpenSSL
fuse.
⚠️ The size-0 fallback cases are real (17 symbols on nginx, 12 on ruby) and
`.ecv.funcs` could not help with them either: it recorded size 0 for them too.
...
Its removal is guarded by `assertNoRedundantFuncTable` in `e2e/e2e_test.go`, which also requires the other `.ecv.*` sections to be present so the absence check cannot pass on a truncated read.
