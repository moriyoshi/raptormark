...
Operating notes. The census is UNSOUND by construction: a skipped instruction means every result after the first skip may be garbage, so only the `addr=` lines carry information, and a clean-looking run is the most dangerous outcome. It is behaviourally inert when it skips nothing -- verified by an interleaved A/B on python with overlapping bands, and by the PostgreSQL arms producing byte-identical output. A build older than the fix reports ONE site and stops: the unhandled SIGILL's default action arms `Pending::Exit` inside `deliver_pending_signals` before it returns, and the run then dies at the next syscall, so the correct shape decides the disposition BEFORE posting and never posts when the census will handle it. Logging is deduplicated by address and capped with an explicit truncation warning; the disposition is decided before signal posting, so census mode without a handler posts no SIGILL and can span syscalls.

Runtime diagnostics must not invent instruction bytes. The arena is populated only from lifter data sections, and `MainLifter::WrapImpl::SetDataSections` excludes `SEC_TYPE_CODE`; therefore `bbmiss` and undecoded messages cannot truthfully read guest `.text`. The former `insn=0x00000000` field was removed after auditing the producer, not merely the consumer. Any future real encoding diagnostic needs a new producer.

The census found zero executed undecoded sites across Python and PostgreSQL, despite 2,244 static PostgreSQL sites. Ruby supplies the actionable counterexample: argv parsing for every `--yjit*` spelling reaches one undecoded ASIMD immediate `orr`. This is the next reachability-proven decoder target; static families such as `st1`, `sli`, and `fcvt` remain unprioritized until a workload reaches them.
